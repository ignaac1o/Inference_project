...
In this section we are going to use sample mean and sample median value as estimators of our population mean.
...
Obviously the sample mean is an unbiased estimator, since $exp(\hat\theta)=\mu$. Meanwhile, as we assume that it's a normal distribution, it's symmetric, thus sample median value is also unbiased $exp(med)=\mu$ and the variance of sample mean is $\hat \sigma^2/n$
...
So far we can see the computation is too complex. However, the variance is given in the slides and others reference as:
...
Since sample mean and sample median value are both unbiased estimators, we can calculate the error by CV, which is: 
...
Since we have obtained the distribution of $\overline x$,we can easily get the variance of the estimator, by CLT, we know that for our variance for a bin(n,p) is $\frac{p(1-p)}{n}$
...
Therefore we can say with a 95% of confidence that between those intervals we have how the electric car market is distributed in terms of their type of drive.
...
As we can see that we have 145 small cars and 32 big cars in the sample, and we assume for each group the distribution is still normal. 
