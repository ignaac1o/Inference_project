...
we are not having the true distribution of the price in population, so we don't know the true variance, thus we don't know the variance of mean value. So we decide to use the method of the pivotal quantity for $\mu$
$$ CI_{1-\alpha}(\mu)=\overline x_n \pm t_{n-1,\alpha/2} \frac{S'}{\sqrt n}$$
```{r echo=FALSE}
CI=c(samplemean+qt(0.025,177)*sqrt(sampleva),samplemean-qt(0.025,i)*sqrt(sampleva))
CI
```
